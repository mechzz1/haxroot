...
var TestController = /** @class */ (function () {
     ...
     */
    TestController.prototype.stripeWebhook = function (req, res, next) {
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        try {
                            ...
                            event = stripe_1.default.getInstance()
                                .getStripe()
                                .webhooks.constructEvent(req.body, sig, globals_1.globals.WEBHOOK_SECRET);
                        }
                        catch (err) {
                            console.error('⚠️ Webhook signature verification failed:', err.message);
                            return [2 /*return*/, res.status(400).send("Webhook Error: ".concat(err.message))];
                        }
                        console.log("[+] Received event: ".concat(event.type));
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, , 5]);
                        if (!(event.type === 'checkout.session.completed')) return [3 /*break*/, 3];
                        return [4 /*yield*/, this.subscriptionService.handleCheckoutCompleted(event.data.object)];
                    case 2:
                        _a.sent();
                        _a.label = 3;
                    case 3: return [2 /*return*/, res.json({ received: true })];
                    case 4:
                        err_1 = _a.sent();
                        console.error('Error processing webhook:', err_1);
    ...
    __decorate([
        decko_1.bind
    ], TestController.prototype, "stripeWebhook", null);
